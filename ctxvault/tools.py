...
retrieve_candidates — BM25 + semantic + graph retrieval via RRF
...
from ctxvault.config import RETRIEVAL_TOP_K
...
class ToolSet:
    ...
    def __init__(
        ...
        *,
        ...
        self._read_counts: Dict[str, List[float]] = {}  # path → [timestamps]
    ...
    def _track_read(self, path: str) -> None:
        ...
        # (approximate: last 2 * ~30s window; here use recent 60s)
        recent = [t for t in ts_list if now - t < 60]
        from ctxvault.config import REREAD_WARN_THRESHOLD
        if len(recent) >= REREAD_WARN_THRESHOLD:
            print(
                f"[WARN] read_file called {len(recent)}× on '{path}' "
                "in the last 60s — possible re-read loop"
            )
    ...
    def retrieve_candidates(
        ...
        """Retrieve top-k candidates via BM25 + semantic + graph (RRF)."""
